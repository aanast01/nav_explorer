...
class Avoider():
	...
	def __init__(self, vel_obj, obstacle_threshold=1.85, 
				       ...
				       trans_lin_vel=12.0, trans_ang_vel=0.0):
		'''
	...
	def indentify_regions(self, scan):
		...
		self.Regions_Report["front_L"] = scan.ranges[-1:2*(int(len(scan.ranges)/len(REGIONS)))+41:-1]
		#print(self.Regions_Report["front_L"])
		
		self.Regions_Report["front_C"] = scan.ranges[2*(int(len(scan.ranges)/len(REGIONS)))+40:int(len(scan.ranges)/len(REGIONS))-40:-1]
		self.MAX_CENTER_LEN = len(scan.ranges[2*(int(len(scan.ranges)/len(REGIONS)))+40:int(len(scan.ranges)/len(REGIONS))-40:-1])
		#print(self.Regions_Report["front_C"])
		
		self.Regions_Report["front_R"] = scan.ranges[int(len(scan.ranges)/len(REGIONS))-41:0:-1]
