    ...
    while not rospy.is_shutdown():
        if flag:
            ...
            angle = math.sqrt(math.pow(
                vel.angular.x, 2) + math.pow(vel.angular.y, 2) + math.pow(vel.angular.z, 2))

            if(angle > 0):
                data.pose.orientation.x = dronePos.pose.orientation.x + \
                    vel.angular.x * math.sin(angle/2)/angle
                data.pose.orientation.y = dronePos.pose.orientation.y + \
                    vel.angular.y * math.sin(angle/2)/angle
                data.pose.orientation.z = dronePos.pose.orientation.z + \
                    vel.angular.z * math.sin(angle/2)/angle
                data.pose.orientation.w = dronePos.pose.orientation.w + \
                    math.cos(angle/2)
            else:
            ...
            cosy_cosp = 1 - 2 * (data.pose.orientation.y * data.pose.orientation.y +
